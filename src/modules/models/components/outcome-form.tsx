...
} from "../utils/outcome-workspace-values";

export type OutcomeFormValues = Record<string, unknown>;
...
export function OutcomeForm({
  ...
  const workspaceSlug = WORKSPACE_SLUGS.OUTCOME_FORM;
  ...
  const [values, setValues] = useState<OutcomeFormValues>(() => {
    if (mode === "edit" && initial && cachedDefinition) {
      return outcomeToFormValues(initial, cachedDefinition.fields);
    }
    if (cachedDefinition) {
  ...
  useEffect(() => {
    ...
    setValues(
      mode === "edit" && initial
        ? outcomeToFormValues(initial, fieldMap)
